...
#Test
# logistic_map(r = 3.6, N0 = 0.5, timesteps = 20)%>%
#   plot(type = 'line')
```
...
N0 <- seq(from = 0.1, to = 0.3, length.out = 5)
...
**Task 3b:** Modify the code above and see what happens when we change $r$ to 3.6.

```{r}
#Set of initial conditions
N0 <- seq(from = 0.1, to = 0.3, length.out = 5)

#Apply each N0 to logistic_map(r = 3.6)
#lmap_chaos <- 
...
logistic_map(r = 3.6, N0 = random_seed, timesteps = 30)%>%
  tail(20)%>% #remove transient
  order()
```

Let's do a permutation test to confirm if this "randomness" works

**Task 3c.**

```{r}
commA <- rpois(20, lambda = 15)
commB <- rpois(20, lambda = 30)
community_richness <- data.frame(richness = c(commA, commB),
                                 community = rep(c("A", "B"), each = 20))
community_richness%>%
  ggplot(aes(x = community, y = richness))+
  geom_point(position = position_jitter(width = 0.1)) x
